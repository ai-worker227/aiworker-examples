...
Paid aiworker routes as Mastra tools. One import builds a tool per route
from the live OpenAPI catalogue; each call pays over x402 from your own
Base wallet. Copy `src/` into your agent, not this directory listing.

## Install

```sh
npm i aiworker-mastra-tools @mastra/core
```

Then copy `src/` (`catalog.ts`, `schema.ts`, `payer.ts`, `types.ts`,
`tools.ts`) and build the tools with the framework's `createTool` injected
(this package never imports `@mastra/core` itself):
...
one to `createTool()` yourself. The built value is the `{ [id]: tool }`
record `new Agent({ tools })` takes.
...
```sh
AIWORKER_PRIVATE_KEY=0x… pnpm --filter @aiworker/example-mastra example
```
...
- No MCP: plain Mastra tools, not a Model Context Protocol server.
